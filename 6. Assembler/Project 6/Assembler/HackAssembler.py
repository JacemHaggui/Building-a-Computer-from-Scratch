...
# Lookup tables for C-instruction conversion
comp_table = {
    ...
    with open(output_file, "w") as file:
        ...
        for line in asmCode.splitlines():
            ...
            if AsmLine.startswith("(") and AsmLine.endswith(")"):
                ...
                print(f"Found label: {label_name} at line {line_number}")
        ...
        variable_pos = 16  # Start of variable memory space
        ...
        for line in asmCode.splitlines():
            ...
            if AsmLine.startswith("@"):
                ...
                if address.isdigit():
                    ...
                    print(f"A-instruction (numeric): @{address} -> {binary_address}")
                elif(address in symbol_table):
                    binary_address = cv.decimal_to_binary(symbol_table.get(address, 0), 15)
                    print(f"A-instruction (register): @{address} -> {binary_address}")
                elif(address in labels):
                    binary_address = cv.decimal_to_binary(labels.get(address, 0), 15)  
                    print(f"A-instruction (symbol): @{address} -> {binary_address}")
                else:# Declaring a variable
                    label_name = address
                    labels[label_name] = variable_pos
                    binary_address = cv.decimal_to_binary(variable_pos, 15)
                    print(f"Found variable: {label_name} at line {variable_pos}")
                    variable_pos += 1
            ...
            # Handle C-instructions
                ...
                print(f"  Binary: comp={comp_bits}, dest={dest_bits}, jump={jump_bits}")
